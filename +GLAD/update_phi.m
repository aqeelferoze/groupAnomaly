...
for p = 1:N
 ...
 for p = 1: N
    for q = 1: N
          % Change To From  Bernouli to Binomial  
            %f = Y(p,q) * B+(1-Y(p,q))*(1-B);  
            f = Y(p,q)* logs(B)+(nC-Y(p,q))*logs(1-B);
            phiL_new1(p,q,:) = f1(:,p)+f*reshape(phiR(q,p,:),[M,1]);
            phiR_new1(q,p,:) = f1(:,q)+f'*reshape(phiL(p,q,:),[M,1]);
    end
 end
phiL_new = mnormalize(exp(phiL_new1),3);
phiR_new = mnormalize (exp(phiR_new1),3);
