...
for n = 1: repeat_num
    ...
    %     [hyper_para_mmsb,var_para_mmsb] = MMSB.mmsb(data.Y,hyper_para_init,  hyperMaxPre, varMaxPre);

    %     save(strcat('./NewResult/mmsbModel',int2str(M),'_',int2str(n),'.mat'),'hyper_para_mmsb','var_para_mmsb');
    rstname = strcat('./NewResult/mmsbModel',int2str(M),'_',int2str(n),'.mat');
    load(rstname); % get the hyper_para_mmsb, var_para_mmsb
