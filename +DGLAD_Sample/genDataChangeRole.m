function [Xp,Y,Pi,Gp,Rp,Theta] = genDataChangeRole(nNum,nT,alpha,B,initTheta,Beta,sigma,nC,nM)
%GENDATACHANGEROLE Summary of this function goes here
%   Detailed explanation goes here
    ...
    delta = zeros(gNum,rNum);
    for gi = 1:gNum-1
        delta(gi,:) = initTheta(gi+1,:) - initTheta(gi,:);
    end
    delta(gNum,:) = initTheta(1,:) - initTheta(gNum,:);
    delta = delta / (nT-1);
    
    for t=2:nT
        Theta{t} = zeros(gNum,rNum);
        for gi=1:gNum         
            tempTheta(gi,:) = tempTheta(gi,:) + delta(gi,:);
            temp =  exp(tempTheta(gi,:));
