...
function y = mvnpdfex(X, Mu, Sigma, ln_pdf)
% Excelly's modification
...
%                     'SIGMA must be symmetric and positive definite.');
              ...
              ridge = eig(Sigma, 1)*1e-7;
              [R, err] = cholcov(Sigma + eye(size(Sigma))*ridge);
              if err ~= 0
                    error('cannot fix this sigma');
              end
            end
...
y = -0.5*quadform - logSqrtDetSigma - d*log(2*pi)*0.5;
